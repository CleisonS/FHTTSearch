...
## Observação sobre janelas de OLT / NE Manager
Algumas janelas do UNM2000 abertas a partir de uma OLT podem ser expostas pelo Windows como janelas Java/owned windows, às vezes com título incompleto no Win32. Por isso o UNM Inspector combina EnumWindows, UI Automation e uma varredura limitada de filhos candidatos do mesmo processo para localizar janelas com padrões como `NE Manager`, `FH_` e `FH-`. Se a janela ainda não aparecer, desmarque temporariamente “Somente visíveis” e filtre por `FH_`, `NE Manager`, `javaw` ou pelo IP da OLT.


## Diagnóstico de enumeração de janelas
Execute no console, sem depender do PySide6:

```bat
python scripts\test_window_enumeration.py
```

Use a saída para confirmar se `EnumWindows` encontra `UNM2000` e `<OLT> - NE Manager - <IP>` antes de investigar a interface gráfica.

## Execução elevada
Se Win32/UIA retornarem acesso negado ou poucos controles para o UNM2000, compare executando:

```bat
scripts\run_as_admin.bat
```

## Validação manual com NE Manager
1. Abra o UNM2000 e faça login manualmente.
2. Abra uma OLT e confirme a janela `<OLT> - NE Manager - <IP>`.
3. Abra o UNM Inspector e clique em **Atualizar janelas**.
4. Filtre por `NE Manager`, selecione a OLT e clique em **Detectar backends**.
5. Clique em **Inspecionar controles** e valide se a árvore é populada.
6. Abra o **Live Inspector** e posicione o cursor sobre Search, ONU List, Device Tree, ONU Status, Device Name e Physical Address.
7. Exporte o relatório completo ZIP.
