...
from app.utils.win_utils import is_user_admin
...
def matches_window_filter(w: WindowInfo, text: str = "", only_unm: bool = False, only_visible: bool = False) -> bool:
    hay = f"{w.title} {w.process_name} {w.class_name} {w.executable_path} {w.pid} {w.handle} 0x{w.handle:08X}".lower()
...
class WindowService:
    ...
    def list_windows(self) -> list[WindowInfo]:
        if os.name != "nt":
            raise RuntimeError("A enumeração de janelas está disponível somente no Windows.")
        started = time.perf_counter()
        LOG.info(
            "Iniciando enumeração de janelas | python=%s | platform=%s | arch=%s | admin=%s",
        ...
        result = sorted(windows.values(), key=lambda w: (not is_unm_candidate(w), not w.visible, w.title.lower(), w.handle))
        LOG.info(
            "Enumeração concluída: hwnds_validos=%s com_titulo=%s visiveis=%s duracao_ms=%s",
            len(result),
            sum(1 for w in result if w.title),
            sum(1 for w in result if w.visible),
    ...
    def _base_from_hwnd(self, hwnd: int, source: str, parent_handle: int = 0, is_top_level: bool = True) -> WindowInfo:
        ...
        thread_id, pid = safe_get(lambda: win32process.GetWindowThreadProcessId(hwnd), (0, 0), "GetWindowThreadProcessId")
        ...
        return WindowInfo(
            ...
            enabled=bool(safe_get(lambda: win32gui.IsWindowEnabled(hwnd), False, "IsWindowEnabled")),
    ...
    def _list_win32_top_level(self) -> list[WindowInfo]:
        ...
        try:
            ...
            LOG.exception("Falha completa ao executar win32gui.EnumWindows")
            raise RuntimeError(f"Falha ao enumerar janelas com pywin32: {exc}") from exc
        LOG.debug("EnumWindows processou %s janelas top-level", len(out))
        return out
